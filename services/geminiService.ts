...
export const initializeChat = async (notes: NoteData, apiKey: string): Promise<string> => {
  ...
  try {
    ...
    // 呼叫 sendMessage 建立初始 context
    // 注意：gemini-3 系列有時會對大量 PDF 處理較久
    const result = await chatSession.sendMessage({ message: parts });
    
    // 確保不會回傳空字串導致前端判斷為無訊息
    if (!result.text) {
        return "助教已準備就緒，但初始回應為空。請直接輸入「開始測驗」來啟動流程。";
    }
    return result.text;
    ...
    if (errorMsg.includes("401") || errorMsg.includes("API key")) {
        errorMsg = "金鑰驗證失敗 (401)。請檢查您的 API Key 是否正確且有效 (是否有多餘空格?)。";
    } else if (errorMsg.includes("400")) {
        errorMsg = "請求格式錯誤 (400)。可能是 PDF 檔案過大或格式損毀。請嘗試將檔案拆分或僅上傳文字部分。";
    } else if (errorMsg.includes("500") || errorMsg.includes("overloaded")) {
        errorMsg = "模型伺服器繁忙 (503)。請稍後再試。";
    }
...
export const sendMessage = async (message: string): Promise<string> => {
  ...
  try {
    ...
    return response.text || "[系統無回應，請重試]";
  } catch (err: any) {
    console.error("Chat Error:", err);
    throw new Error("傳輸失敗: " + (err.message || "未知錯誤"));
  }
};
